...
void ScalePlane(const uint8* src, int src_stride,
				...
				enum FilterMode filtering) {
					...
					if ((src_width == dst_width) && (src_height > dst_height)){
						int half_off = (src_height - dst_height) >> 1;
						memcpy(dst, src + src_stride*half_off, dst_stride*dst_height);
						return;
					}

					// Straight cut
					if ((src_width > dst_width) && (src_height == dst_height)){
						int half_off = (src_width - dst_width) >> 1;
						unsigned char* point_dst = dst;

						for (int i = 0; i < dst_height; i++){
							memcpy(point_dst, src + i * src_stride + half_off, dst_stride);
							point_dst = point_dst + dst_stride;
						}
						return;
					}
					...
					if (dst_width <= Abs(src_width) && dst_height <= src_height) {
						if (src_width * dst_height == src_height * dst_width){
							...
							int new_src_height = (int)((src_width * dst_height *1.0) / (dst_width * 1.0) + 0.5);
							int new_src_width = src_width;
							int half_off = (src_height - new_src_height) >> 1;
							ScalePlaneBox(new_src_width, new_src_height, dst_width, dst_height,
								src_stride, dst_stride, src + half_off * src_stride, dst);
							return;
						}
						else if (src_width * dst_height > src_height * dst_width){
							int new_src_width = (int)((src_height * dst_width *1.0) / (dst_height * 1.0) + 0.5);
							int new_src_height = src_height;
							int half_off = (src_width - new_src_width) >> 1;
							ScalePlaneBox(new_src_width, new_src_height, dst_width, dst_height,
								src_stride, dst_stride, src + half_off, dst);
							return;
						}
					}
					...
					if (dst_width > Abs(src_width) && dst_height > src_height) {
						if (src_width * dst_height == src_height * dst_width){
							...
							int new_src_width = src_width;
							int half_off = (src_height - new_src_height) >> 1;
							ScalePlaneBilinearUp(new_src_width, new_src_height, dst_width, dst_height,
								src_stride, dst_stride, src + half_off * src_stride, dst, filtering);
							return;
						}
						else if (src_width * dst_height > src_height * dst_width){
							int new_src_width = (int)((src_height * dst_width *1.0) / (dst_height * 1.0) + 0.5);
							...
							int half_off = (src_width - new_src_width) >> 1;
							ScalePlaneBilinearUp(new_src_width, new_src_height, dst_width, dst_height,
								src_stride, dst_stride, src + half_off, dst, filtering);
							return;
						}
					}
...
// scale video stream according to original aspect ratio,fill the up boundary and bottom boundary of video stream 
LIBYUV_API
int I420PadScale_vertical(const uint8* src_y, int src_stride_y,
	...
	int new_dst_width = dst_width;
	int new_dst_height = src_height * new_dst_width / src_width;
	int offset_threshold = 2;

	if (dst_height - new_dst_height <= offset_threshold)
		...
		int dst_height_half = (dst_height + 1) >> 1;
		int new_dst_height_half = (new_dst_height + 1) >> 1;

		int y_offset_Y = (dst_height - new_dst_height + 1) >> 1;
		int y_offset_U = (dst_height_half - new_dst_height_half + 1) >> 1;
		int y_offset_V = (dst_height_half - new_dst_height_half + 1) >> 1;

		uint8 *point_Y = dst_y + y_offset_Y*dst_stride_y;
		uint8 *point_U = dst_u + y_offset_U*dst_stride_u;
		uint8 *point_V = dst_v + y_offset_V*dst_stride_v;

		I420Scale(src_y, src_stride_y,
			      src_u, src_stride_u,
			      src_v, src_stride_v,
			      src_width, src_height,
			      point_Y, dst_stride_y,
			      point_U, dst_stride_u,
			      point_V, dst_stride_v,
			      new_dst_width, new_dst_height,
			      filtering);

		// fill the top bar and bottom bar of the Y_plane
		memset(dst_y, 16, dst_stride_y*y_offset_Y);
		memset(dst_y + (y_offset_Y + new_dst_height)*dst_stride_y, 16, dst_stride_y*(dst_height - y_offset_Y - new_dst_height));

		// fill the top bar and bottom bar of the U_plane
		memset(dst_u, 128, dst_stride_u*y_offset_U);
		memset(dst_u + (y_offset_U + new_dst_height_half)*dst_stride_u, 128, dst_stride_u*(dst_height_half - y_offset_U - new_dst_height_half));

		// fill the top bar and bottom bar of the V_plane
		memset(dst_v, 128, dst_stride_v*y_offset_V);
		memset(dst_v + (y_offset_V + new_dst_height_half)*dst_stride_v, 128, dst_stride_v*(dst_height_half - y_offset_V - new_dst_height_half));

		return 0;
	}
}

// scale video stream according to original aspect ratio,fill the left boundary and right boundary of video stream 
LIBYUV_API
int I420PadScale_horizontal(const uint8* src_y, int src_stride_y,
                            ...
                            int dst_width, int dst_height,
                            FilterMode filtering){
	int new_dst_height = dst_height;
	int new_dst_width = src_width * new_dst_height / src_height;
	int offset_threshold = 2;

	if (dst_width - new_dst_width <= offset_threshold)
		return I420Scale(src_y, src_stride_y,
		                 src_u, src_stride_u,
		                 src_v, src_stride_v,
		                 src_width, src_height,
		...
		int dst_width_half = (dst_width + 1) >> 1;
		int dst_height_half = (dst_height + 1) >> 1;
		int new_dst_width_half = (new_dst_width + 1) >> 1;

		int x_offset_Y = (dst_width - new_dst_width + 1) >> 1;
		int x_offset_U = (dst_width_half - new_dst_width_half + 1) >> 1;
		int x_offset_V = (dst_width_half - new_dst_width_half + 1) >> 1;

		uint8 *point_Y = dst_y + x_offset_Y;
		uint8 *point_U = dst_u + x_offset_U;
		uint8 *point_V = dst_v + x_offset_V;
		...
		// fill the left bar and right bar of the Y plane
		uint8 *fill_point_Y = dst_y;
		for (int ii = 0; ii < dst_height; ii++)
		{
			memset(fill_point_Y, 16, x_offset_Y);
			fill_point_Y = fill_point_Y + dst_stride_y;
		}
		fill_point_Y = dst_y + x_offset_Y + new_dst_width;
		for (int jj = 0; jj < dst_height; jj++)
		{
			memset(fill_point_Y, 16, dst_width - new_dst_width - x_offset_Y);
			fill_point_Y = fill_point_Y + dst_stride_y;
		}

		// fill the left bar and right bar of the U plane
		uint8 *fill_point_U = dst_u;
		for (int ii = 0; ii < dst_height_half; ii++)
		{
			memset(fill_point_U, 128, x_offset_U);
			fill_point_U = fill_point_U + dst_stride_u;
		}
		fill_point_U = dst_u + x_offset_U + new_dst_width_half;
		for (int jj = 0; jj < dst_height_half; jj++)
		{
			memset(fill_point_U, 128, dst_width_half - new_dst_width_half - x_offset_U);
			fill_point_U = fill_point_U + dst_stride_u;
		}

		// fill the left bar and right bar of the V plane
		uint8 *fill_point_V = dst_v;
		for (int ii = 0; ii < dst_height_half; ii++)
		{
			memset(fill_point_V, 128, x_offset_V);
			fill_point_V = fill_point_V + dst_stride_v;
		}
		fill_point_V = dst_v + x_offset_V + new_dst_width_half;
		for (int jj = 0; jj < dst_height_half; jj++)
		{
			memset(fill_point_V, 128, dst_width_half - new_dst_width_half - x_offset_V);
			fill_point_V = fill_point_V + dst_stride_v;
		}
